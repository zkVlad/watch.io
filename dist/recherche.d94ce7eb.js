...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"assets/js/recherche.js":[function(require,module,exports) {
...
var formulaireDeRecherce = document.querySelector('.formulaire-de-recherce');
var listeDesResultats = document.getElementById('resultats-recherche');
var zonePagination = document.getElementById('zone-pagination');
var presentationResultats = document.getElementById('presentation-resultats');
var chargement = document.createElement('div');
var nombreDeResultats = 0;
chargement.classList.add('chargement');
var appelApi = function appelApi(requete, page) {
  ...
  return fetch("".concat(requete, "&page=").concat(page)).then(function (response) {
    return response.json();
  }).then(function (data) {
    chargement.remove();
    var filmsRetournes = data.Search;
    filmsRetournes.forEach(function (film) {
      var itemFilm = document.createElement('li');
      itemFilm.classList.add('item-film');
      var posterFilm = document.createElement('img');
      if (film.Poster === 'N/A') {
      ...
      var titreFilm = document.createElement('h2');
      titreFilm.textContent = film.Title;
      var anneeFilm = document.createElement('p');
      anneeFilm.textContent = film.Year;
      var typeFilm = document.createElement('span');
      switch (film.Type) {
        case 'movie':
    ...
    var headingPresentationResultats = document.createElement('h2');
    headingPresentationResultats.textContent = 'Resultats de votre recherche :';
    var nombrePresentationResultats = document.createElement('p');
    ...
    if (nombreDeResultats > 10) {
      ...
      var totalPages = Math.ceil(nombreDeResultats / 10);
      var nombrePageAffichees = 5;
      var pagination = document.createElement('div');
      pagination.id = 'pagination';
      var boutonPrecedent = document.createElement('button');
      ...
      var indicationPages = document.createElement('div');
      indicationPages.textContent = "sur ".concat(totalPages);
      indicationPages.classList.add('indication-pages');
      var boutonSuivant = document.createElement('button');
      ...
      var numerosPages = document.createElement('div');
      ...
      goTo.addEventListener("keyup", function (event) {
        if (event.key === "Enter") {
...
formulaireDeRecherce.addEventListener('submit', function (e) {
  e.preventDefault();
  var champTitreFilm = formulaireDeRecherce.querySelector('input[name="titre"]');
  var champAnneeFilm = formulaireDeRecherce.querySelector('input[name="annee"]');
  var champTypeFilm = formulaireDeRecherce.querySelector('input[name="item"]:checked');
  var valeurTitreFilm = champTitreFilm.value;
  var valeurAnneeFilm = champAnneeFilm.value;
  var requete = "http://www.omdbapi.com/?apikey=4ee808d0&s=".concat(valeurTitreFilm, "&y=").concat(valeurAnneeFilm);
  if (champTypeFilm) {
    var valeurTypeFilm = champTypeFilm.dataset.requete;
    requete = requete + "&type=".concat(valeurTypeFilm);
  }
  appelApi(requete, 1);
});
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","assets/js/recherche.js"], null)
//# sourceMappingURL=/recherche.d94ce7eb.js.map