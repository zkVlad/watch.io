...
var createRow = function createRow(obj) {
  ...
  var caseBoutonSupprimer = document.createElement("td");
  var boutonSupprimer = document.createElement("button");
  boutonSupprimer.textContent = "Supprimer";
  boutonSupprimer.classList.add('bouton', 'bouton--warning', 'bouton__supprimer');
  caseBoutonSupprimer.appendChild(boutonSupprimer);
  row.appendChild(caseBoutonSupprimer);
  return row;
};
var getTableContent = function getTableContent(data) {
  ...
  var boutonsSupprimer = _toConsumableArray(document.querySelectorAll(".bouton__supprimer"));
  boutonsSupprimer.map(function (bouton) {
    bouton.addEventListener("click", supprimmerRang);
  });
};
var retournerMajuscules = function retournerMajuscules(str) {
  return str.split(" ").map(function (word) {
    return word.charAt(0).toUpperCase() + word.slice(1);
  }).join(" ");
};
var supprimmerRang = function supprimmerRang(e) {
  _sweetalert.default.fire({
    showCloseButton: true,
    html: "\n            <h2>Souhaitez-vous vraiment supprimer \"".concat(e.target.parentNode.parentNode.firstChild.textContent, "\"</h2>\n        "),
...
var sortData = function sortData(data, param) {
  var direction = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "asc";
  tableContent.innerHTML = '';
  var sortedData = direction == "asc" ? _toConsumableArray(data).sort(function (a, b) {
    if (a[param] < b[param]) {
      return -1;
    }
    if (a[param] > b[param]) {
...
var resetButtons = function resetButtons(event) {
  _toConsumableArray(tableButtons).map(function (button) {
...
/* Rajout films */
var boutonAjoutFilm = document.getElementById("bouton-ajout-film");
boutonAjoutFilm.addEventListener("click", function () {
  _sweetalert.default.fire({
    showCloseButton: true,
    html: "\n            <h2>Ajouter un film \xE0 votre liste</h2>\n            <input type=\"text\" id=\"champ-titre-film\" class=\"swal2-input\" placeholder=\"Titre du film\">\n            <input type=\"number\" id=\"champ-annee-film\" class=\"swal2-input\" placeholder=\"Ann\xE9e de sortie\">\n            <input type=\"text\" id=\"champ-auteur-film\" class=\"swal2-input\" placeholder=\"Nom de l'auteur\">\n        ",
    confirmButtonText: 'Ajouter le film',
    focusConfirm: false,
    preConfirm: function preConfirm() {
      var champTitreFilm = retournerMajuscules(_sweetalert.default.getPopup().querySelector('#champ-titre-film').value);
      var champAnneeFilm = retournerMajuscules(_sweetalert.default.getPopup().querySelector('#champ-annee-film').value);
      var champAuteurFilm = retournerMajuscules(_sweetalert.default.getPopup().querySelector('#champ-auteur-film').value);
      ...
      if (!/^\d{4}$/.test(champAnneeFilm) || champAnneeFilm < 1900 || champAnneeFilm > anneeCourante) {
        erreursSaisie.push("Indiquez une ann\xE9e dans un format \xE0 4 chiffres et comprise en 1900 et ".concat(anneeCourante, "."));
      }
      ...
      if (erreursSaisie.length > 0) {
        var erreursFormatees = erreursSaisie.map(function (erreur) {
          return "<li>".concat(erreur, "</li>");
        }).join('');
        _sweetalert.default.showValidationMessage("\n                <p>Votre saisie est incorrecte :</p>\n                <ul>".concat(erreursFormatees, "</ul>\n            "));
        setTimeout(function () {
          _sweetalert.default.resetValidationMessage();
        }, 6000);
      }
      return {
        champTitreFilm: champTitreFilm,
        champAnneeFilm: champAnneeFilm,
        champAuteurFilm: champAuteurFilm
      };
    }
  }).then(function (result) {
    var nouveauFilm = [{
...
var champSelectTri = document.querySelector("#choix-filtres");
champSelectTri.addEventListener('change', function () {
  var optionSelectionnee = this.options[this.selectedIndex];
  var idBoutonTri = optionSelectionnee.dataset.boutonVise;
  var boutonTri = document.querySelector("#".concat(idBoutonTri));
  _toConsumableArray(tableButtons).map(function (button) {
    if (button !== boutonTri) {
      button.removeAttribute("data-dir");
    }
